// Vize seçim aracı karar ağacı verisi. Web vize-secim/VizeClient.tsx'ten
// birebir kopyalanmıştır (plan §13.2). Option.next bir sonraki soru id'sini
// ya da 'RESULT:<id>' biçiminde sonucu işaret eder.

export interface VisaOption { key: string; label: string; next: string; }
export interface VisaQuestion { id: string; text: string; hint?: string; options: VisaOption[]; }
...
export const RESULTS: VisaResult[] = [
  // ── YENİ SONUÇLAR ─────────────────────────────────────────────────────────
  {
    id: 'SCHENGEN',
