// Tatil hesaplama motoru. Kaynak: referans/lib/utils/holiday-calculator.ts

import { TR_HOLIDAY_MAP } from './tr-2026';
import { createHolidayMap, type GermanStateCode } from './de-2026';
...
export function calculateTRRange(startDate: Date, endDate: Date): CalculationResult {
  ...
  for (let d = new Date(start); d.getTime() <= end.getTime(); d = addDaysUTC(d, 1)) {
    ...
    days.push({ date: iso, weekend, holiday: holiday ? { name: holiday.name, weight: holiday.weight } : null });
  }

  const leaveDays = Math.max(0, workdays - holidayOnWorkdays);
  return { totalDays, weekendDays, workdays, officialHolidayDays: round1(holidayOnWorkdays), leaveDays: round1(leaveDays), holidayHits, days };
}

export function calculateDERange(startDate: Date, endDate: Date, stateCode: GermanStateCode): CalculationResult {
  ...
  for (let d = new Date(start); d.getTime() <= end.getTime(); d = addDaysUTC(d, 1)) {
    ...
    days.push({ date: iso, weekend, holiday: holiday ? { name: holiday.name, weight: 1 } : null });
  }
...
export function groupDaysByWeek(days: DayInfo[]): WeekGroup[] {
  ...
  for (const day of days) {
    ...
    if (day.holiday) {
      ...
      bucket.holidays.push({ date: day.date, name: day.holiday.name, weight: day.holiday.weight, weekend: day.weekend });
    }
...
function newWeekBucket(key: string, startIso: string): WeekGroup {
  return { key, start: startIso, end: startIso, totalDays: 0, weekendDays: 0, holidayDaysAll: 0, holidayDaysWork: 0, halfHolidayDays: 0, holidays: [], leaveDays: 0, bridgeHint: false };
}
...
export function calculateEfficiency(leaveDays: number, totalDays: number): {
  ...
  if (leaveDays <= 0) {
    return { percentage: 100, ratio: 0, label: '0 gün izinle: tamamen hafta sonu / resmî tatil', badge: '🌿 Tam "resmî tatil + hafta sonu" modu' };
  }
  ...
  return { percentage, ratio: round1(ratio), label: `1 gün izin ≈ ${round1(ratio)} gün tatil`, badge };
}
